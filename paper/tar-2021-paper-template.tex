...
%While humans easily learn commonsense knowledge, until the advent of transformer models, that has proved to be a difficult task for machines
%the best one yields an accuracy of $95.6\%$, which is comparable to state-of-the-art results.
% However, 
% recently, with the advent  of  transformer  models
...
%Most adult humans are equipped with common sense i.e., according to Cambridge Dictionary, the ability to use good judgment in making decisions and to live in a reasonable and safe way. Even though common sense knowledge is mostly basic, it is not a fundamental principle of the Universe but rather a collection of knowledge built by generations of humans through observations of phenomena in their surroundings. Since humans are not born with common sense, they start learning it from the earliest stages of their life, either explicitly or implicitly.
...
%\citet{wang-etal-2019-make} state that most standard NLU benchmarks verify whether a system is equipped with common sense by testing whether it can give a correct answer where the input does not contain such knowledge. However, they argue these benchmarks do not give a direct metric to quantitatively measure sense-making capability and do not explicitly identify the key factors required in a sense-making process. They address these issues by creating a test set for direct sense-making used for SemEval-2020 Task 4, Commonsense Validation and Explanation (ComVE) \citep{wang-etal-2020-semeval} which consists of three subtasks, namely Validation, Explanation (Multi-Choice), and Explanation (Generation).
...
% The most successful approaches to the Validation subtask \cite{zhang-etal-2020-cn,zhao-etal-2020-ecnu} leverage external commonsense knowledge stored in the ConceptNet \cite{conceptnet}. However, the success of the ConcepNet-based systems may be attributed to the data leakage which may have occurred due to the fact that ConceptNet was used as an inspiration for ComVE data generation along with transformer-based models. Rather than leveraging the existing knowledge bases, we attempt to generate additional knowledge artificially by employing text augmentation techniques.

%TODO: Šta je u kojem sectionu, malo bolje naglasit da je glavni fokus rada ova GPT augmentacija, a možda i ne treba

\section{Related work}
\label{sec:rw}
% The most challenging NLU task nowadays is the Winograd Schema Challenge (WSC) \cite{winograd-levesque,winograd-morgenstern}. Designed to be an improvement on the Turing test, it is a multiple-choice test that employs questions of a very specific structure. For example, \textit{``The town councilors refused to give the demonstrators a permit because they feared (advocated) violence. Who feared (advocated) violence? Answer 0: the councilors, Answer 1: the demonstrators.``} However, WSC estimates commonsense indirectly and it does not consider an explanation on why one option is true while the other is wrong. It also largely requires extra common sense knowledge.

% An example of common sense causal reasoning is Choice of Plausible Alternatives (COPA) \citep{COPA}, whose main attention lies upon events and consequences. Each question of COPA asks to find the most reasonable cause or result of the premise from two given alternatives. Both the premise and the two alternatives are always simple sentences. For example, \textit{``Premise: I knocked on my neighbor's door. What happened as a RESULT? Alternative 1: My neighbor invited me in. Alternative 2: My neighbor left his house.``} COPA has prepared the ground for several other datasets like the JHU Ordinal Common-sense Inference (JOCI) \citep{JOCI} that aims to label the soundness from 5 (very likely) to 1 (impossible) of human response after a certain situation. Those datasets put emphasis solely on whether some situations occur before and/or after some certain situation, rather than explaining why they occur.
...
\section{Common sense validation model} % Obavezno spomenut reseursna ograničenja
\label{model} % Ovaj inspired by dio nije nužan ja mislim. Može se samo krenut od ovog our model consists............. jer su u principu svi tak pristupili
%Inspired by \citep{wang-etal-2020-semeval},
...
%and selecting the model with respect to validation loss for evaluation on the test set. % while also saving the best model according to the least validation loss, so it can then be used for testing step.

\begin{figure} % @Vedran: mozemo koristit figure* ak ocete da bude kroz oba columna i mozda width drukciji
    \centering
...
\label{aug}
% We find text augmentation an interesting topic since, unlike image augmentation, it does not receive much attention in research, especially in NLU tasks. 
...
%the value given by hyperparameter \texttt{p}. In our work we set \texttt{p} to 0.9.
...
% On the other hand, geometric mean is not affected by sentence length, allowing for selection of longer sentences, which is is demonstrated in Figure \ref{fig:cum}. This can explain the 
\begin{figure}
    \centering
...
%Ovo poglavlje možda neće trebat ak se navedu svi detalji u opisu modela (hiperparametri, trajanje treniranja...)

\section{Results} % Nema do ansambla
\label{results}
...
% In this paper, we have showed that preprocessing data for the humor ranking task brings no statistical significance to  performance as the tweets are already small to train character-level models on. Furthermore, we show that the best-performing model on the task is a character-level CNN that is able to determine the funnier of any two tweets on a given topic with a 70.4% probability on the evaluation dataset. This result is currently the highest for the humor ranking task and represents state-of-the-art performance. Nevertheless, we see room for further improvement. Firstly, it would be interesting to implement some external knowledge since a lot of the puns are related to the actual movies, people or events. Also, including features concerning Twitter specifics could be beneficiary for our model’s performance since tweets without them have higher probability to be awarded a place in the top 10 funniest.

...
def augment_B(B_df, augmenter):
    """
    Function that augments subtask B data
    :param B_df: subtask B data
    :param augmenter: augmenter used to augment data
    :return: pandas DataFrame with augmented data
    """

    s1 = augmenter.augment(list(B_df['FalseSent'].values))
    s2 = augmenter.augment(list(B_df['OptionA'].values))
    s3 = augmenter.augment(list(B_df['OptionB'].values))
    s4 = augmenter.augment(list(B_df['OptionC'].values))

    return pd.DataFrame({'FalseSent': s1, 'OptionA': s2, 'OptionB': s3, 'OptionC': s4})
